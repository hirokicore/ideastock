...
export async function refineStock(
  stock: IdeaStock,
  context?: {
    variations?: { title: string; shared_core: string }[];
    relatedStocks?: { title: string; summary: string | null }[];
  }
): Promise<RefineResult> {
  const variationsText = context?.variations?.length
    ? `\nバリエーション（統合済みサブ案）:\n${context.variations.map((v) => `・${v.title}（共通コア: ${v.shared_core}）`).join('\n')}`
    : '';

  const relatedText = context?.relatedStocks?.length
    ? `\n関連アイデア:\n${context.relatedStocks.map((r) => `・${r.title}${r.summary ? `：${r.summary.slice(0, 80)}` : ''}`).join('\n')}`
    : '';

  const prompt = `あなたは個人開発者のアイデア精査AIです。
以下のアイデアを「市場性・実現性・継続性」の3軸で限界まで問い詰め、スコア100点（Sランク）を目指した改善版を生成してください。
現状の弱点を遠慮なく指摘し、独自性・競合優位性・マネタイズ経路を最大化してください。
...
${stock.human_note ? `一言メモ: ${stock.human_note}` : ''}${variationsText}${relatedText}
...
【出力JSON仕様】
{
  ...
  "recommend_score": 改善後の総合スコア（100点Sランクを目指す、0〜100の整数）,
